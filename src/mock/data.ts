...
export const systemHardware: SystemHardware[] = [
  ...gateways.map((gateway, index) => ({
    id: gateway.id,
    name: `${gateway.room} 蓝牙采集网关`,
    type: "BLE 网关" as const,
    model: "GAIS-BGW-200",
    serialNumber: `BGW26${String(1001 + index)}`,
    room: gateway.room,
    position: ["西侧入口", "东侧入口", "西侧走廊", "中部吊顶", "东侧走廊"][
      index % 5
    ],
    ip: gateway.ip,
    port: gateway.port,
    online: gateway.online,
    connectedDevices: gateway.devices,
    heartbeat: gateway.heartbeat,
    uptime: gateway.uptime,
    firmware: index < 2 ? "v2.4.1" : "v2.3.8",
    owner: index % 2 ? "刘洋" : "陈工",
  })),
  {
    id: "LOC-2F-01",
